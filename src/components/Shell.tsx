...
import { Button, Modal, Toast, IconLink, IconBox, IconTrend, IconDownload, IconArchive, IconGear, IconPlus, IconCheck, IconWarn, IconSync, IconChevronRight, fmtDT } from './ui';
...
const NAV = [
  { to: '/connect', label: '接入同步码', icon: <IconLink size={15} /> },
  { to: '/tasks', label: '任务库', icon: <IconBox size={15} /> },
  { to: '/trends', label: '库存趋势', icon: <IconTrend size={15} /> },
  { to: '/downloads', label: '下载中心', icon: <IconDownload size={15} /> },
  { to: '/archive', label: '档案', icon: <IconArchive size={15} /> },
  { to: '/settings', label: '设置', icon: <IconGear size={15} /> },
];

const TITLES: Record<string, string> = {
  '/connect': '接入同步码', '/tasks': '任务库', '/trends': '库存趋势', '/downloads': '下载中心', '/archive': '档案', '/settings': '设置',
};
...
export function SyncModal() {
  ...
  return (
    <Modal
      open={open} onClose={close} title="接入同步码" width={600}
      footer={<>
        ...
        <Button onClick={run} disabled={busy || codes.length === 0} icon={busy ? <IconSync size={13} spinning /> : <IconLink size={13} />}>
          {busy ? '正在拉取数据包' : codes.length > 1 ? `接入 ${codes.length} 个任务` : '接入'}
        </Button>
      </>}
    >
      <div style={{ fontSize: 12.5, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
        在手机端「巡检结果 → 结果报告 → 同步到网页端」复制同步码，粘贴到下方。一次可粘贴多个，用换行或逗号分隔。
      </div>
      <textarea
      ...
      <div className="mt-1.5 flex items-center justify-between" style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>
        <span>同步码有效期 7 天，超期请在手机端重新生成</span>
        ...
        <div className="mt-4 flex flex-col gap-1.5">
          {results.map((r, i) => (
            <div key={i} className="flex items-center gap-3" style={{ padding: '10px 12px', borderRadius: 8, background: r.ok ? 'var(--success-bg)' : 'var(--danger-bg)' }}>
              ...
              <span className="flex-1 truncate" style={{ fontSize: 12.5, color: r.ok ? 'var(--text-primary)' : 'var(--danger)' }}>
                {r.ok ? `${r.already ? '已在任务库中' : '已接入'} · ${r.task.routeName} · ${fmtDT(r.task.startedAt)}` : r.reason}
              </span>
              {r.ok && (
          ...
          {current && (
            <div className="flex items-center gap-3" style={{ padding: '10px 12px', borderRadius: 8, background: 'var(--brand-subtle-bg)' }}>
              ...
              <span style={{ fontSize: 12.5, color: 'var(--brand-subtle-text)' }}>校验同步码，从机载端拉取数据包</span>
            </div>
          )}
          {!busy && results.length > 1 && (
            <div className="mono mt-1" style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>{downloadRecordsOk}/{results.length} 个接入成功</div>
          )}
        </div>
      )}
    </Modal>
    ...
    <div className="flex" style={{ minHeight: '100vh' }}>
      ...
      <aside className="no-print shrink-0 flex flex-col" style={{ width: 'var(--sidebar-w)', background: 'var(--surface-1)', borderRight: '1px solid var(--border-subtle)', position: 'sticky', top: 0, height: '100vh' }}>
        ...
        <div style={{ padding: '0 12px 12px' }}>
          <Button style={{ width: '100%' }} icon={<IconPlus size={13} />} onClick={() => set({ syncModalOpen: true })}>接入同步码</Button>
        </div>
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        <header className="no-print flex items-center shrink-0" style={{ height: 54, padding: '0 28px', borderBottom: '1px solid var(--border-subtle)', background: 'var(--glass-bar)', backdropFilter: 'blur(10px)', position: 'sticky', top: 0, zIndex: 20 }}>
          ...
          <span className="ml-auto mono" style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>手机端数据经同步码接入 · 本地保留 90 天</span>
        </header>
